...
// ====== Trees ======================================
  ...
  object Tree {
    case class TreeStatement(stat: Statement) extends Tree
    // XXX according to TastyFormat, pkg should be a path.
...
// ------ Statements ---------------------------------
  ...
  enum Statement {
    case Import(expr: Term, selector: List[ImportSelector])
    case TermStatement(expr: Term)
    case DefinitionStatement(definition: Definition)
  }
...
// ------ Definitions ---------------------------------
  ...
  case class TypeDef(name: String, rhs: TypeTree | Tree.TypeBoundsTree) extends Definition {
    def mods: List[Modifier] = ???
  }
...
// ------ Terms ---------------------------------
  ...
  enum Term {
    ...
    case Match(scrutinee: Term, cases: List[Tree.CaseDef])
    case Try(body: Term, catches: List[Tree.CaseDef], finalizer: Option[Term])
